    ...
    class Matrix {
        public:
            ...
            Matrix<T> operator[](const size_t& i) const {
                // underlying functionality that fetches the row elements
                // catches invalid row index.
                return Matrix<T>(row(i));
            }
            ...
            template<arithmetic_type T1, arithmetic_type T2>
                friend Matrix<Common_type<T1,T2>> operator*(
                            const Matrix<T1>&, const Matrix<T2>&
                        );
